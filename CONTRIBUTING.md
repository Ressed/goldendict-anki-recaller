...
项目使用 uv 管理 Python 依赖与 `.venv`，采用 Python 3.10+。真实 Anki 引擎测试需匹配所安装 Anki 使用的 Python 版本。
...
uv run --frozen --python 3.13 python -m unittest tests.test_priority_queue -v
```

本项目曾在 Anki 26.8.1 / Python 3.13 上验证该集成测试。普通查词与模板测试使用 Python 3.14。请勿用真实学习集合运行测试。

修改队列逻辑时，请覆盖原生作答、撤销和不同每日限额；修改网页交互时，验证多词条隔离、默认选中、暂停授权和重复提交保护。不要提交 `config.json`、`.venv`、生成的预览、学习计划或集合数据库。
